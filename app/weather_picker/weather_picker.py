...
from app.exceptions import PickerClientError, PickerConnectionError
...
class WeatherPicker(ABC):
    ...
    def receive_weather_data(
        self, api_key: str
    ) -> list[WeatherDataSchemeFromOpenWeather]:
        ...
...
class WeatherPickerWithSubscription(WeatherPicker):
    ...
    async def fetch_weather(
        aio_session: ClientSession, current_url: str
    ...
    async def receive_weather_data(
        ...
        """Note that subscription gives asynchronous weather
        collecting, so it's faster"""
        ...
        async with ClientSession() as session:
            try:
                ...
                logger.critical(
                    "Some error occurred while picking weather. \n",
                    exc_info=client_err,
                )
...
class WeatherPickerWithoutSubscription(WeatherPicker):
    ...
    def receive_weather_data(
        ...
        try:
            ...
            logger.error(
                "Connection refused or there is some HTTP error", exc_info=req_err
            )
